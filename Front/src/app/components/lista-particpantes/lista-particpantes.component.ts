...
export class ListaParticpantesComponent implements OnInit {
  ...
  obtenerRegistro() {
    this._registroService.getRegistro().subscribe(
      (data) => {
        console.log(data);
  ...
  getTipoEvento(registro: any): string {
    if (registro.conferenciaId && registro.conferenciaId !== '') {
      return 'Conferencia';
    }
    if (registro.tallerId && registro.tallerId !== '') {
      return 'Taller';
    }
    return 'Sin evento';
  }
